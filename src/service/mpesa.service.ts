...
import { Logger, OrderService, Payment, RequestContext, TransactionalConnection } from "@vendure/core";
import { MpesaPluginOptions } from "../mpesa.plugin";
import { CALLBACK_URL_ENDPOINT, LIVE_BASE_URL, MPESA_PLUGIN_INIT_OPTIONS, SANDBOX_BASE_URL, loggerCtx } from "../constants";
...
export class MpesaService {
    ...
    constructor(
        @Inject(MPESA_PLUGIN_INIT_OPTIONS) private pluginOptions: MpesaPluginOptions,
    ...
    async initiateStkPush(amount: number, phoneNumber: string, orderCode: string) {
        ...
        const transactionType = shortCodeType === "paybill" ? "CustomerPayBillOnline" : "CustomerBuyGoodsOnline";
        const timestamp = this.getCurrentTimestamp();

        const { data } = await client.post<STKPushResponse>('/stkpush/v1/processrequest', {
    ...
    async updateTransactionStatus(ctx: RequestContext, transactionId: string) {
        ...
        try {
            const { data } = await client.post<STKStatusResponse>('/stkpushquery/v1/query', {
            ...
            if (data.ResponseCode === "0" && data.ResultCode === "0") {
                Logger.info(`Transaction ${transactionId} was successful`, loggerCtx);
                ...
                Logger.error(`Transaction ${transactionId} failed with error ${data.ResultDesc}`, loggerCtx);
                await this.paymentStateMachine.transition(
                    ctx,
                    order,
                    payment,
            ...
            Logger.error(`Couldn't query transaction ${transactionId} status`, loggerCtx);
        }
    }
    ...
    private async getPaymentByTransactionId(ctx: RequestContext, transactionId: string): Promise<Payment> {
